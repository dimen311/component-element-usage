...
var FileFinder = class {
  ...
  async findFiles() {
    ...
    for (const file of files) {
      const filePath = path2.join(this.roothPath || "", file);
...
function activate(context) {
  let disposable = vscode4.commands.registerCommand(
    ...
    async () => {
      ...
      vscode4.window.registerTreeDataProvider(
  ...
  let openFileCommand = vscode4.commands.registerCommand("extension.openFileAtLine", async (filePath, lineNumber) => {
    await vscode4.commands.executeCommand("vscode.open", filePath, {
    ...
    const editor = vscode4.window.activeTextEditor;
    if (editor) {
      const position = new vscode4.Position(lineNumber - 1, 0);
