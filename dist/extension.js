...
var ComponentIndexManager = class _ComponentIndexManager {
  ...
  extractComponentSelector(content) {
...
var FileFinder = class {
  ...
  searchedSelector;
  indexManager;
  constructor(filePath, roothPath, context) {
  ...
  async findFiles() {
    ...
    for (const usage of usages) {
      const filePath = path3.join(this.roothPath, usage.file);
  ...
  async getFileContent(filePath) {
    const content = await import_vscode.default.workspace.fs.readFile(
      import_vscode.default.Uri.file(filePath)
    );
    return Buffer.from(content).toString("utf8");
  }
  extractComponenSelector(fileContent) {
    const regex = /@Component\(\s*{[^}]*?\bselector:\s*'([^']*)'/;
...
// src/file-tree-data-provider.ts
var import_vscode3 = __toESM(require("vscode"));
var path4 = __toESM(require("path"));

// src/file-tree.ts
var import_vscode2 = __toESM(require("vscode"));
var FileTreeItem = class extends import_vscode2.default.TreeItem {
  constructor(filePath, label, lineNumber, children, options) {
    super(
      label,
      children === void 0 ? import_vscode2.default.TreeItemCollapsibleState.None : import_vscode2.default.TreeItemCollapsibleState.Expanded
    );
    this.filePath = filePath;
    this.label = label;
    ...
    if (children) {
      this.command = {
        command: "vscode.open",
        title: "Open File",
        arguments: [import_vscode2.default.Uri.file(this.filePath)]
      };
    } else {
      this.command = {
        command: "extension.openFileAtLine",
        title: "Open File",
        arguments: [import_vscode2.default.Uri.file(this.filePath), this.lineNumber]
      };
    }
  }
};
...
var FileTreeDataProvider = class {
  _onDidChangeTreeData = new import_vscode3.default.EventEmitter();
  ...
  getChildren(element) {
    if (this.isLoading) {
      return [new FileTreeItem("", "Loading...", null, void 0, {
        iconPath: new import_vscode3.default.ThemeIcon("loading~spin")
      })];
    }
    if (!element) {
...
function activate(context) {
  ...
  const fileWatcher = vscode5.workspace.createFileSystemWatcher("**/*.{ts,html}", false, false);
  ...
  let disposable = vscode5.commands.registerCommand("component_element_usage.findComponentUsage", async () => {
    ...
    const workspaceFolder = vscode5.workspace.getWorkspaceFolder(document.uri);
    if (!workspaceFolder) {
      vscode5.window.showInformationMessage("No workspace folder found");
      return;
    }
    ...
    setTimeout(() => {
      ...
      vscode5.commands.executeCommand("workbench.view.explorer");
      vscode5.commands.executeCommand("elementUsageExplorer.focus");
    }, 100);
  });
  context.subscriptions.push(disposable);
  let openFileCommand = vscode5.commands.registerCommand("extension.openFileAtLine", async (filePath, lineNumber) => {
    await vscode5.commands.executeCommand("vscode.open", filePath, {
    ...
    const editor = vscode5.window.activeTextEditor;
    if (editor) {
      const position = new vscode5.Position(lineNumber - 1, 0);
      const range = new vscode5.Range(position, position);
